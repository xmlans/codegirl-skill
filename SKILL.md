...
| 解析 GitHub/代码片段 | `Bash` → `python3 ${CLAUDE_SKILL_DIR}/tools/github_parser.py` |
| 解析 Telegram 聊天记录导出 | `Bash` → `python3 ${CLAUDE_SKILL_DIR}/tools/telegram_parser.py` |
...
参考 `${CLAUDE_SKILL_DIR}/prompts/memory_builder.md` 生成 Coding Memory 内容。
...
路径：`gfs/{slug}/memory.md`

**3. 写入 persona.md**（用 Write 工具）：
路径：`gfs/{slug}/persona.md`

**4. 写入 meta.json**（用 Write 工具）：
路径：`gfs/{slug}/meta.json`
内容：
...
1. 按 Step 2 的方式读取新内容
2. 用 `Read` 读取现有 `gfs/{slug}/memory.md` 和 `persona.md`
